...
@Suite internal struct ConditionalsExampleTests {
  ...
  internal func testCompletedConditionalsExample() throws {
    ...
    let program = Group {
      ...
      If(Let("actualNumber", "Int(possibleNumber)"), then: {
        Call("print") {
          ParameterExp(name: "", value: "\"The string \\\"\\(possibleNumber)\\\" has an integer value of \\(actualNumber)\"")
        }
      }, else: {
        Call("print") {
          ParameterExp(name: "", value: "\"The string \\\"\\(possibleNumber)\\\" could not be converted to an integer\"")
        }
      })
    ...
    generated = generated.replacingOccurrences(of: ":\\s*\\w+\\s*=", with: "=", options: .regularExpression)
      .normalize()

    // Load expected Swift from example file
    let projectRoot = URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
    let expectedURL = projectRoot.appendingPathComponent("Examples/Completed/conditionals/code.swift")
